...
do
	...
	if [[ -z $broker_fd ]]
	then
		# connect failed, retry at most 5 times(dynamically get a file descriptor from /dev/tcp and store it to variable broker_fd)
		max_retry_count=5
		...
		echo "----connect success:  at broker_fd: $broker_fd /dev/tcp/$host/$port-----"
		retry_connection_counter=0
		...
		do
			...
			then
				...
				then
				    #* if server send self-play command to worker 
