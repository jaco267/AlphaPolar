...
if [[ ${run_stage,} == "r" ]]; then
	...
	echo "----config file: ${sp_executable_file} -gen ${train_dir}/${new_configure_file} -conf_file ${configure_file} -conf_str "${overwrite_conf_str}" 2>/dev/null----"
	...
	echo "I guess this is just run one times to create folder without any training!!!!? python ${op_executable_file} ${game_type} ${train_dir} ${train_dir}/${new_configure_file}"
...
echo "---run zero server script :  ${sp_executable_file} -conf_file ${train_dir}/${new_configure_file} -conf_str ${conf_str} -mode zero_server----"
