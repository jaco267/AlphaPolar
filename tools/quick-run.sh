...
log INFO "scripts/build.sh $game"    
...
if [[ $mode == train ]]; then # ================================ TRAIN ================================
    ...
    if [[ ! $conf_file ]]; then
        ...
        log WARN "conf_file $conf_file"  
        log WARN "git info $git_info" 
        log WARN "gen cof file : $executable -gen $conf_file >/dev/null 2>&1"
        # if conf_file exist  or gen conf_file dont show output and warn 
    ...
    declare -A PID
    log INFO "------running script for launch zero server:  scripts/zero-server.sh $game $conf_file $zero_end_iteration -n "$train_dir" -g ${CUDA_VISIBLE_DEVICES//,/} -conf_str "$conf_str"-----"
    # launch zero server
    {
    ...
    #*---- wait until the initialization done, read go {eval_dir}/Training.log, and check its time----
    tick=$(date +%s%3N)
    sleep 2
    ...
    for GPU in ${SP_CUDA_VISIBLE_DEVICES//,/ }; do
        {
            ...
            log INFO "------running script for launch zero sp workers: scripts/zero-worker.sh $game $(hostname) $zero_server_port sp -b ${batch_size:-64} -c ${num_threads:-4} -g $GPU ${sp_conf_str:+-conf_str "$sp_conf_str"}----"
    ...
    {
        log INFO "------running script for launch zero op worker: scripts/zero-worker.sh $game $(hostname) $zero_server_port op -g ${OP_CUDA_VISIBLE_DEVICES//,/} ${op_conf_str:+-conf_str "$op_conf_str"}------"
    ...
    log WARN "---launch $launch  ---executable  $executable---"
    log WARN "---eval_dir $eval_dir----"
    log WARN "---nnfilename $nn_file_name----"
    ...
    if [[ ! $port ]]; then # launch minizero console directly
        ...
        log WARN "we are in 1"
        log INFO "$launch $executable -mode console -conf_file ${conf_file} -conf_str "${conf_str}""
        {
