...
public:
    ...
    Puzzle2048Action(int action_id, Player player) : 
       BaseAction(action_id, player) {}
    Puzzle2048Action(const std::vector<std::string>& 
       action_string_args){
        assert(action_string_args.size() && action_string_args[0].size());
        action_id_ = std::string("URDL").find(
            std::toupper(action_string_args[0][0]));
    ...
    inline std::string toConsoleString() const { 
        return kPuzzle2048ActionName[
            std::min<unsigned>(action_id_, 4)]; }
};
...
public:
    ...
    bool act(const std::vector<std::string>& action_string_args,
     bool with_chance = true) override { 
        return act(Puzzle2048Action(action_string_args), 
          with_chance); }
    ...
    int getMaxChanceEventSize() const override { 
        return 32; }
    float getChanceEventProbability(
        const Puzzle2048Action& action) const override;
    ...
    int getRotatePosition(int position, 
      utils::Rotation rotation) const override { 
        return utils::getPositionByRotating(rotation, position, kPuzzle2048BoardSize); }
    int getRotateAction(int action_id, 
      utils::Rotation rotation) const override;
    ...
    inline int getNumInputChannels() const override {
         return 16; }
    inline int getNumActionFeatureChannels() const override { return 1; }
    inline int getInputChannelHeight() const override { 
        return kPuzzle2048BoardSize; }
    inline int getInputChannelWidth() const override { 
        return kPuzzle2048BoardSize; }
    inline int getHiddenChannelHeight() const override { 
        return kPuzzle2048BoardSize; }
    inline int getHiddenChannelWidth() const override { 
        return kPuzzle2048BoardSize; }
    inline int getPolicySize() const override { 
        return kPuzzle2048ActionSize; }
    inline int getDiscreteValueSize() const override { 
        return kPuzzle2048DiscreteValueSize; }
...
public:
    std::vector<float> getActionFeatures(const int pos, 
      utils::Rotation rotation = utils::Rotation::kRotationNone) 
      const override { 
        return Puzzle2048Env().getActionFeatures(
            pos < static_cast<int>(action_pairs_.size()) ? 
           action_pairs_[pos].first : Puzzle2048Action(), rotation); }
    std::vector<float> getValue(const int pos) const override { 
        return toDiscreteValue(pos < static_cast<int>(action_pairs_.size()) ? 
             utils::transformValue(calculateNStepValue(pos)) : 0.0f); }
    ...
    std::vector<float> getReward(const int pos) const override { 
        return toDiscreteValue(pos < static_cast<int>(action_pairs_.size()) ? 
        utils::transformValue(BaseEnvLoader::getReward(pos)[0]) : 0.0f); }
    float getPriority(const int pos) const override { 
        return fabs(calculateNStepValue(pos) - 
         BaseEnvLoader::getValue(pos)[0]); }

    std::string name() const override { return kPuzzle2048Name; }
    int getPolicySize() const override { return 4; }
    int getRotatePosition(int position, utils::Rotation rotation) 
      const override { 
        return utils::getPositionByRotating(rotation, position, kPuzzle2048BoardSize); }
    int getRotateAction(int action_id, utils::Rotation rotation) 
      const override { 
        return Puzzle2048Env().getRotateAction(action_id, rotation); }
