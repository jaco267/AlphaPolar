...
bool Puzzle2048Env::act(const Puzzle2048Action& action, 
   bool with_chance /* = true */){
...
bool Puzzle2048Env::actChanceEvent(){ //* done set a rand pos
    if (turn_ != Player::kPlayerNone) { return false; }
    int empty = board_.countEmptyPositions();
    if (empty == 0) { return false; }
    int pos = board_.getNthEmptyPosition(
        std::uniform_int_distribution<int>(0, empty - 1)(random_));
...
bool Puzzle2048Env::isLegalAction(const Puzzle2048Action& action) const{
    ...
    Bitboard(board_).slide(action.getActionID()) != -1 && 
    action.getPlayer() == Player::kPlayer1;
}

bool Puzzle2048Env::isLegalChanceEvent(
    const Puzzle2048Action& action) const{
    Puzzle2048ChanceEvent event = Puzzle2048ChanceEvent::toChanceEvent(action);
...
std::vector<float> Puzzle2048Env::getFeatures(
    utils::Rotation rotation 
    /*= utils::Rotation::kRotationNone*/) const{
  // 16 channels: the nth channel represents the position of the nth tile
  std::vector<float> features;
  for (int tile = 0; tile < 16; ++tile) {
    for (int pos = 0; pos < 16; ++pos) {
      features.push_back(board_.get(
        getRotatePosition(pos, rotation)) == tile ? 
           1.0f : 0.0f);
    }
  }
  return features;
...
Puzzle2048Action Puzzle2048Env::Puzzle2048ChanceEvent::toAction(
    const Puzzle2048Env::Puzzle2048ChanceEvent& event){
    if (event.pos_ == -1 || event.tile_ == -1) return {};
    return {(event.tile_ << 4) | (event.pos_), Player::kPlayerNone};
}
std::string Puzzle2048Env::toString() const{ //* done
    std::ostringstream oss; oss << board_;
    return oss.str();
}
