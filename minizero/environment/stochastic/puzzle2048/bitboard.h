...
public:
    ...
    Bitboard(const Bitboard& b) = default;  //copy constructor?
...
private:
    ...
    struct RowLookup {
      ...
      static int calculateSlideLeft(int row[]){
        int top = 0; int tmp = 0; int score = 0;
        for (int i = 0; i < 4; i++) {
          int tile = row[i];
          if (tile == 0) continue;
          row[i] = 0;
          if (tmp != 0) {
            if (tile == tmp) {
              tile = tile + 1; row[top++] = tile;
              score += (1 << tile);
              tmp = 0;
            } else {
              row[top++] = tmp; tmp = tile;
            }
          } else { tmp = tile;}
        }
...
public:
    ...
    void initialize(bool add_init_tiles = true){
        raw_ = 0;
        if (add_init_tiles) { popup();popup();}
    }
    /**add a new rand tile on board,or do nothing if board full
     * 2-tile: 90%  * 4-tile: 10%*/
    bool popup(int rand = std::rand()){ //* called by initialize
    ...
    enum SlideDirection {
        kUp = 0, kRight = 1, kDown = 2, kLeft = 3
    };
    /***apply an action to the board,return reward gained by 
     * action, or -1 if action is illegal*/
    int slide(int opcode){
        switch (opcode) {
            case 0:return slideUp();
            case 1:return slideRight();
            case 2:return slideDown();
            case 3:return slideLeft();
            default:return -1;
        }
    }
    int slideLeft(){
    ...
    void transpose(){
        raw_ = (raw_ & 0xf0f00f0ff0f00f0fULL) | (
               (raw_ & 0x0000f0f00000f0f0ULL) << 12) | (
               (raw_ & 0x0f0f00000f0f0000ULL) >> 12);
        raw_ = (raw_ & 0xff00ff0000ff00ffULL) | (
               (raw_ & 0x00000000ff00ff00ULL) << 24) | (
               (raw_ & 0x00ff00ff00000000ULL) >> 24);
    }
...
public:
    ...
    uint32_t getNthEmptyPosition(uint32_t n) const{
        ...
        uint64_t nth = x;
        while (n--) nth &= nth - 1;
        //      returns the number of trailing zeroes in nth.
        return __builtin_ctzll(nth) / 4;
    }
public:
  friend std::ostream& operator<<(
    std::ostream& out, const Bitboard& b){
    char buff[32];
    out << "+------------------------+" << std::endl;
    for (int i = 0; i < 16; i += 4) {
      std::snprintf(buff, sizeof(buff), "|%6u%6u%6u%6u|",
        (1 << b.get(i + 0)) & -2u, (1 << b.get(i + 1)) & -2u,
        (1 << b.get(i + 2)) & -2u, (1 << b.get(i + 3)) & -2u);
      out << buff << std::endl;
    }
    out << "+------------------------+" << std::endl;
    return out;
  }
private: uint64_t raw_;
};
