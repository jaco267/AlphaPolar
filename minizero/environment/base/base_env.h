...
enum class Player {
    kPlayerNone = 0, kPlayer1 = 1, kPlayer2 = 2, kPlayerSize = 3
};
...
public:
    ...
    inline void set(Player p, const T& value) { 
        (p == Player::kPlayer1 ? black_ : white_) = value; }
...
public:
    ...
    virtual bool act(const std::vector<std::string>& 
        action_string_args) = 0;
    ...
    virtual std::vector<float> getFeatures(
        ...
        utils::Rotation::kRotationNone) const = 0;
    virtual std::vector<float> getActionFeatures(
        const Action& action, 
        utils::Rotation rotation = 
        utils::Rotation::kRotationNone) const = 0;
    ...
    virtual int getRotatePosition(int position, 
        utils::Rotation rotation) const = 0;
    virtual int getRotateAction(int action_id, 
        utils::Rotation rotation) const = 0;
...
public:
  virtual void reset(){
      sgf_content_.clear();
      tags_.clear(); tags_.insert({"GM", name()});
      tags_.insert({"RE", "0"});
      action_pairs_.clear();
  }
  ...
  virtual bool loadFromString(const std::string& content){
    reset(); sgf_content_ = content;
    std::string key, value;
    int state = '(';
    bool accept_move = false;  bool escape_next = false;
    int board_size = minizero::config::env_board_size;
    for (char c : content) {
      switch (state) {
        case '(': // wait until record start
          if (!accept_move) {accept_move = (c == '(');
          } else { state = (c == ';') ? c : 'x';  accept_move = false;}
          break;
        case ';': // store key
          if (c == ';') { accept_move = true;
          } else if (c == '[' || c == ')') {state = c;
          } else if (std::isgraph(c)) { key += c;}
          break;
        case '[': // store value
          if (c == '\\' && !escape_next) { escape_next = true;
          } else if (c != ']' || escape_next) {
              value += c; escape_next = false;
          } else { // ready to store key-value pair
            if (accept_move) {
              int action_id = value.size() && std::isdigit(value[0]) ? std::stoi(value) : 
                        minizero::utils::SGFLoader::sgfStringToActionID(value, board_size);
            ...
            key.clear();  value.clear(); state = ';';
          }
          break;
        case ')': break;// end of record, do nothing
      }
    }
    return state == ')';
  }
  virtual void loadFromEnvironment(const Env& env, 
      const std::vector<std::vector<std::pair<std::string, std::string>>>& action_info_history = {}){
      reset();
  ...
  virtual std::vector<float> getFeatures(const int pos, 
      utils::Rotation rotation = utils::Rotation::kRotationNone) const{
      // slow but naive method which simply replays game again to get features
      Env env;
      for (int i = 0; i < std::min(pos, 
        static_cast<int>(action_pairs_.size()));
         ++i) {
        env.act(action_pairs_[i].first); 
      }
      return env.getFeatures(rotation);
    }
  virtual std::vector<float> getPolicy(
    const int pos, utils::Rotation rotation = utils::Rotation::kRotationNone
  ) const{
  ...
  virtual std::vector<float> getValue(const int pos) const { 
      return (pos < static_cast<int>(action_pairs_.size()) ? 
        std::vector<float>{std::stof(action_pairs_[pos].second["V"])} : 
  ...
  virtual bool setActionPairInfo(const int pos, 
  ...
  virtual std::vector<float> getActionFeatures(const int pos, 
      utils::Rotation rotation = utils::Rotation::kRotationNone) const = 0;
  virtual std::string name() const = 0;
  virtual int getPolicySize() const = 0;
  ...
  virtual inline std::string getTag(const std::string& key) const { 
    return tags_.count(key) ? tags_.at(key) : ""; }
  ...
  inline const std::vector<std::pair<Action, ActionInfo>>& getActionPairs() const { 
      return action_pairs_; }
  inline void addActionPair(const Action& action, const ActionInfo& action_info = {}) { 
      action_pairs_.emplace_back(action, action_info); }
  inline float getReturn() const { return std::stof(getTag("RE")); }
protected:
    std::string escapeSGFString(const std::string& str) const{
        std::string special = "()[]\\";   std::string escaped;
...
protected:
    std::string sgf_content_; Tags tags_;
...
public:
    ...
    BaseBoardAction(const std::vector<std::string>& action_string_args,
         int board_size = minizero::config::env_board_size){
        ...
        action_id_ = minizero::utils::SGFLoader::boardCoordinateStringToActionID(
            action_string_args[1], board_size);
    }
...
public:
    ...
    void loadFromEnvironment(const Env& env, 
