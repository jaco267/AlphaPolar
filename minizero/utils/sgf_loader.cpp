...
int SGFLoader::boardCoordinateStringToActionID(const std::string& board_coordinate_string, int board_size)
{
    ...
    int x = std::toupper(board_coordinate_string[0]) - 'A' + 
            (std::toupper(board_coordinate_string[0]) > 'I' ? -1 : 0);
    int y = atoi(board_coordinate_string.substr(1).c_str()) - 1;
    return y * board_size + x;
}

std::string SGFLoader::actionIDToBoardCoordinateString(
    int action_id, int board_size){
