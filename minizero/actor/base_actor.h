...
public:
    ...
    virtual std::string getRecord(const std::unordered_map<std::string, std::string>& 
        tags = {}) const;
    ...
    inline const std::vector<std::vector<std::pair<std::string, std::string>>>& 
      getActionInfoHistory() const { return action_info_history_; }
    //* virtual functions
    virtual Action think(bool with_play = false, bool display_board = false) = 0;
    virtual void beforeNNEvaluation() = 0;
    virtual void afterNNEvaluation(const std::shared_ptr<network::NetworkOutput>& 
        network_output) = 0;
