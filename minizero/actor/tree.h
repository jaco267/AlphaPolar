...
public:
    ...
    inline virtual TreeNode* getChild(int index) const { 
        return (index < num_children_ ? first_child_ + index : nullptr); }
...
class Tree {
public:
    Tree(uint64_t tree_node_size) : tree_node_size_(tree_node_size),nodes_(nullptr){
    ...
    inline TreeNode* allocateNodes(int size){
        ...
        //* initialy current_node_size = 1
        TreeNode* node = getNodeIndex(current_node_size_); //getRootNode+index   ex. root_node +1
        // std::cout<<"current_node_size: "<< current_node_size_<<" , ";
    ...
    std::string getTreeInfo_r(const TreeNode* node) const{
        std::ostringstream oss; int numChildren = 0;
