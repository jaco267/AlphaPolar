...
public:
    ...
    std::pair<int, int> calculateTrainingDataRange(
        const std::shared_ptr<BaseActor>& actor);
    bool do_cpu_job_; int actor_index_; std::mutex mutex_;
    std::vector<std::shared_ptr<BaseActor>> actors_;
    std::vector<std::shared_ptr<network::Network>> networks_;
    std::vector<std::vector<std::shared_ptr<network::NetworkOutput>>> 
       network_outputs_;
};
...
public:
    ...
    void initialize() override;  void runJob() override;
    bool isDone() override { return false; }
protected:
    virtual bool doCPUJob();  virtual void doGPUJob();
    virtual void handleSearchDone(int actor_id);
    inline std::shared_ptr<ThreadSharedData> getSharedData() { 
        return std::static_pointer_cast<ThreadSharedData>(shared_data_); }
};
class ActorGroup : public utils::BaseParalleler {
public:
    ActorGroup() {}
    void run();
    void initialize() override;  void summarize() override {}
protected:
    virtual void createNeuralNetworks();   virtual void createActors();
    virtual void handleIO();   virtual void handleCommand();
    ...
    std::shared_ptr<utils::BaseSlaveThread> newSlaveThread(int id) override { 
        return std::make_shared<SlaveThread>(id, shared_data_); }
    inline std::shared_ptr<ThreadSharedData> getSharedData() { 
        return std::static_pointer_cast<ThreadSharedData>(shared_data_); }
    bool running_;
