...
std::string GumbelZero::getMCTSPolicy(const std::shared_ptr<MCTS>& mcts) const
{
    ...
    float non_visited_node_value = 1.0 / (1 + config::actor_num_simulation) * 
                                    (value_pi + (config::actor_num_simulation / pi_sum) * q_sum);
    ...
    for (int i = 0; i < mcts->getRootNode()->getNumChildren(); ++i) {
        ...
        float score = logit_without_noise + (
            config::actor_gumbel_sigma_visit_c + max_child_count
           ) * config::actor_gumbel_sigma_scale_c * value;
...
std::vector<MCTSNode*> GumbelZero::selection(const std::shared_ptr<MCTS>& mcts)
{
    ...
    if (mcts->getNumSimulation() == 0) {
        ...
        sort(candidates_.begin(), candidates_.end(), 
          [](const MCTSNode* lhs, const MCTSNode* rhs) {
...
void GumbelZero::sequentialHalving(const std::shared_ptr<MCTS>& mcts)
{
    if (mcts->getNumSimulation() == 1) {
        ...
        candidates_.clear();
        for (int i = 0; i < mcts->getRootNode()->getNumChildren(); ++i) { 
            candidates_.push_back(mcts->getRootNode()->getChild(i)); 
        }
        sort(candidates_.begin(), candidates_.end(), 
        ...
        simulation_budget_ = std::max(1.0, std::floor(
            config::actor_num_simulation / (std::log2(config::actor_gumbel_sample_size) * sample_size_)
        ));
    } else {
        ...
        if (all_candidates_reach_budget) {
            int next_budget = std::floor(config::actor_num_simulation / 
                (std::log2(config::actor_gumbel_sample_size) * sample_size_ / 2));
...
void GumbelZero::sortCandidatesByScore(const std::shared_ptr<MCTS>& mcts)
{
    ...
    float max_child_count = 0;
    for (int i = 0; i < mcts->getRootNode()->getNumChildren(); ++i) { 
        max_child_count = fmax(max_child_count, mcts->getRootNode()->getChild(i)->getCount()); }
    ...
    sort(candidates_.begin(), candidates_.end(), [&](const MCTSNode* lhs, const MCTSNode* rhs) {
        ...
        float lhs_score = lhs->getPolicyLogit() + (config::actor_gumbel_sigma_visit_c + max_child_count) * 
                                                   config::actor_gumbel_sigma_scale_c * lhs_value;
        lhs_score = (lhs->getCount() > 0 ? lhs_score : min_value);
        float rhs_value = rhs->getNormalizedMean(tree_value_bound);
        float rhs_score = rhs->getPolicyLogit() + (config::actor_gumbel_sigma_visit_c + max_child_count) * 
                                                   config::actor_gumbel_sigma_scale_c * rhs_value;
