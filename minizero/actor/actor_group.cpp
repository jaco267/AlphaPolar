...
void ThreadSharedData::outputGame(const std::shared_ptr<BaseActor>& actor){
    ...
    oss << "SelfPlay " << (is_terminal ? "true" : "false") << " " // is terminal
...
std::pair<int, int> ThreadSharedData::calculateTrainingDataRange(
    const std::shared_ptr<BaseActor>& actor
){
    int game_length = actor->getEnvironment().getActionHistory().size();
...
void SlaveThread::runJob(){
    if (getSharedData()->do_cpu_job_) {  while (doCPUJob()) {}
    } else {doGPUJob();}
}
bool SlaveThread::doCPUJob(){
    ...
    if (network_output_id >= 0) {
      assert(network_output_id < static_cast<int>(
          getSharedData()->network_outputs_[network_id].size()));
      actor->afterNNEvaluation(
        getSharedData()->network_outputs_[network_id]
                                         [network_output_id]);
      if (actor->isSearchDone()) { 
        // cout<<"end at doCPUJob"<<endl;
        handleSearchDone(actor_id); }
    }
    /* 8x8 board    
    simulation budget = 3   
    current node size 1   ->  
    action_1 expand: action_size= 8 ->   current_node_size= 1+8 = 9   
    action_2 expand: valid_action_size=  8-1=7 -> current_node_size= 9 + 7 = 16   
    action_3 expand: valid_action_size= 7-1 = 6 -> current_node_size = 16 + 7 = 23 
    */
...
void SlaveThread::doGPUJob(){
    ...
    if (network->getNetworkTypeName() == "alphazero") {
        std::shared_ptr<AlphaZeroNetwork> az_network = 
           std::static_pointer_cast<AlphaZeroNetwork>(network);
        if (az_network->getBatchSize() > 0) {
             getSharedData()->network_outputs_[id_] = az_network->forward(); }
...
void SlaveThread::handleSearchDone(int actor_id){
    assert(actor_id >= 0 && actor_id < static_cast<int>(
        getSharedData()->actors_.size()) && getSharedData()->actors_[actor_id]->isSearchDone());
    ...
    if (is_endgame) {
        ...
        if (sequence_length > 0 && game_length >= sequence_length && (game_length - config::learner_n_step_return - config::learner_muzero_unrolling_step) % sequence_length == 0) { 
            getSharedData()->outputGame(actor); }
        actor->resetSearch();
    }
}
...
void ActorGroup::initialize(){
    int num_threads = std::max(static_cast<int>(torch::cuda::device_count()), 
                              config::zero_num_threads);
    createSlaveThreads(num_threads);  createNeuralNetworks();
    createActors();   running_ = false;  getSharedData()->do_cpu_job_ = true;
    ...
    std::vector<std::string> ignored_commands = utils::stringToVector(
        config::zero_actor_ignored_command);
    for (const auto& command : ignored_commands) { ignored_commands_.insert(command); }
}
void ActorGroup::createNeuralNetworks(){ //* half create networks
    int num_networks = std::min(static_cast<int>(torch::cuda::device_count()), 
                               config::zero_num_parallel_games);
    ...
    for (int gpu_id = 0; gpu_id < num_networks; ++gpu_id) {
        getSharedData()->networks_[gpu_id] = createNetwork(config::nn_file_name,
             gpu_id);
    }
}
void ActorGroup::createActors(){ //* half create actors   
    assert(getSharedData()->networks_.size() > 0);
    std::shared_ptr<Network>& network = getSharedData()->networks_[0];
    uint64_t tree_node_size = static_cast<uint64_t>(config::actor_num_simulation + 1) 
               * network->getActionSize();
    for (int i = 0; i < config::zero_num_parallel_games; ++i) {
        getSharedData()->actors_.emplace_back(
            createActor(tree_node_size, 
                getSharedData()->networks_[i % getSharedData()->networks_.size()])
        );
    }
}
void ActorGroup::handleIO(){  //* todo
    std::string command;
...
void ActorGroup::handleCommand(){//*half  call handleCommand(command_prefix..)
...
void ActorGroup::handleCommand(const std::string& command_prefix, //todo
