...
using namespace minizero;  using namespace network;
void MCTSSearchData::clear(){
    search_info_ = "";  selected_node_ = nullptr;
    node_path_.clear();
}
void ZeroActor::reset(){
    BaseActor::reset();
    enable_resign_ = (
      utils::Random::randReal() < config::zero_disable_resign_ratio ? false : true);
}

void ZeroActor::resetSearch(){
    BaseActor::resetSearch();
    mcts_search_data_.node_path_.clear();
    getMCTS()->getRootNode()->setAction(Action(-1, 
        env::getPreviousPlayer(env_.getTurn(), env_.getNumPlayer())));
}

Action ZeroActor::think(bool with_play /*= false*/, 
  ...
  while (!isSearchDone()) {
    ...
    if (config::actor_mcts_think_time_limit > 0 && 
      spent_million_second >= 
      config::actor_mcts_think_time_limit * 1000) { break; }
  }
  if (!isSearchDone()) { handleSearchDone(); }
  if (with_play) { act(getSearchAction()); }
  if (display_board) { std::cerr << env_.toString() 
    << mcts_search_data_.search_info_ << std::endl; }
...
void ZeroActor::beforeNNEvaluation(){
  ...
  if (alphazero_network_) {
    ...
    feature_rotation_ = config::actor_use_random_rotation_features ? 
        static_cast<utils::Rotation>(utils::Random::randInt() % 
                  static_cast<int>(utils::Rotation::kRotateSize)) : 
            utils::Rotation::kRotationNone;
    nn_evaluation_batch_id_ = alphazero_network_->pushBack(
      ...
      if (getMCTS()->getNumSimulation() == 0) { // initial inference for root node
          ...
          nn_evaluation_batch_id_ = muzero_network_->pushBackRecurrentData(
              hidden_state, env_.getActionFeatures(leaf_node->getAction()));
      }
  } else {
      assert(false);
  }
}
void ZeroActor::afterNNEvaluation(
  const std::shared_ptr<NetworkOutput>& network_output){
  const std::vector<MCTSNode*>& node_path = mcts_search_data_.node_path_;
  MCTSNode* leaf_node = node_path.back();
  if (alphazero_network_) {
    ...
    if (!env_transition.isTerminal()) {
      std::shared_ptr<AlphaZeroNetworkOutput> alphazero_output = 
        std::static_pointer_cast<AlphaZeroNetworkOutput>(network_output);
        // cout<<"policy..."<<alphazero_output
        // cout<<"policy size:" << alphazero_output->policy_.size()<<endl;
      getMCTS()->expand(leaf_node, 
        calculateAlphaZeroActionPolicy(env_transition, 
                alphazero_output, feature_rotation_));
      getMCTS()->backup(node_path, 
        alphazero_output->value_, env_transition.getReward());
    } else {
      getMCTS()->backup(node_path, 
        env_transition.getEvalScore(), 
        env_transition.getReward());
    }
  } else if (muzero_network_) {
...
void ZeroActor::setNetwork(const std::shared_ptr<network::Network>& network){ //* almost done
    assert(network); alphazero_network_ = nullptr; muzero_network_ = nullptr;
...
std::vector<std::pair<std::string, std::string>> ZeroActor::getActionInfo() const{
    ...
    if (getMCTS()->getRootNode()->getCount() > 0) { 
        return BaseActor::getActionInfo(); }
    return {};
}
...
void ZeroActor::step(){
    ...
    int batch_size = std::min(config::actor_mcts_think_batch_size,
        (alphazero_network_ || num_simulation > 0) ? num_simulation_left : 1 
        /* initial inference for root node */);
    assert(batch_size > 0);

    std::vector<std::tuple<int, utils::Rotation, 
       decltype(mcts_search_data_.node_path_)>> batch_queries; // batch id, rotation, search path
    for (int batch_id = 0; batch_id < batch_size; batch_id++) {
        beforeNNEvaluation();
        assert(nn_evaluation_batch_id_ == batch_id);
        // cout<<"...!!!"<<nn_evaluation_batch_id_<<"..."<<batch_id<<endl;
        if (mcts_search_data_.node_path_.back()->getVirtualLoss() == 0) {
            batch_queries.emplace_back(nn_evaluation_batch_id_, 
              feature_rotation_, mcts_search_data_.node_path_);
        }
    ...
    for (auto& query : batch_queries) {
        ...
        for (auto node : mcts_search_data_.node_path_) { 
...
void ZeroActor::handleSearchDone(){
    ...
    if (config::actor_mcts_value_rescale) { 
        oss << ", value bound: (" << getMCTS()->getTreeValueBound().begin()->first << ", " << getMCTS()->getTreeValueBound().rbegin()->first << ")"; }
    oss << std::endl
        << "  root node info: " 
        << getMCTS()->getRootNode()->toString() << std::endl
        << "action node info: " 
        << mcts_search_data_.selected_node_->toString() << std::endl;
    mcts_search_data_.search_info_ = oss.str();
}
MCTSNode* ZeroActor::decideActionNode(){//*called by self.handleSearchDone
  if (config::actor_use_gumbel) {
    ...
    if (config::actor_select_action_by_count) {
        ...
        return getMCTS()->selectChildBySoftmaxCount(
            getMCTS()->getRootNode(), 
            config::actor_select_action_softmax_temperature);
    }
    assert(false);
    return nullptr;
  }
...
void ZeroActor::addNoiseToNodeChildren(MCTSNode* node){
    ...
    if (config::actor_use_dirichlet_noise) {
        ...
        std::vector<float> gumbel_noise = utils::Random::randGumbel(
            node->getNumChildren());
        for (int i = 0; i < node->getNumChildren(); ++i) {
            MCTSNode* child = node->getChild(i);
...
std::vector<MCTS::ActionCandidate> ZeroActor::calculateAlphaZeroActionPolicy(
    const Environment& env_transition, 
    const std::shared_ptr<network::AlphaZeroNetworkOutput>& alphazero_output, 
    ...
    for (size_t action_id = 0; action_id < alphazero_output->policy_.size(); ++action_id) {
        ...
        action_candidates.push_back(MCTS::ActionCandidate(
            action, alphazero_output->policy_[rotated_id], 
            alphazero_output->policy_logits_[rotated_id]));
    }
    sort(action_candidates.begin(), action_candidates.end(), [](
        const MCTS::ActionCandidate& lhs, const MCTS::ActionCandidate& rhs) {
