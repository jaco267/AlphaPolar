...
//* added  
int kern_search_game_len = 100;
std::string network_type = "conv"; 
int rand_init_step = 3;  
std::string min_max_comp = "-1,-1";
...
void setConfiguration(ConfigureLoader& cl)
{
    ...
    //* added  
    cl.addParameter("network_type",network_type, "transformer or conv network", "Network");
    // environment parameters
    cl.addParameter("env_board_size", env_board_size, "the size of board", "Environment");
    //* added
    cl.addParameter("kern_search_game_len", kern_search_game_len, "kern search max game len", "Environment");
    cl.addParameter("rand_init_step", rand_init_step, "randomly push this number of stone on the 2nd row after reset to add more data diversity", "Environment");
    cl.addParameter("min_max_comp",min_max_comp, "est min comp and max comp for complexity reward", "Environment");
#if ATARI
