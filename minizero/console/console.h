...
enum class ConsoleResponse : char {kFail = '?',kSuccess = '='};
class Console {
public:
    Console();  virtual ~Console() = default;
...
protected:
    ...
    public:
        Function(I* instance, F function) : 
           instance_(instance), function_(function) {}
        void operator()(const std::vector<std::string>& args) { 
            (*instance_.*function_)(args); }
        I* instance_;  F function_;
    };
    template <class I, class F>
    void RegisterFunction(const std::string& name, 
        I* instance, F function){
