...
Console::Console(): network_(nullptr),actor_(nullptr){
    ...
    RegisterFunction("gen_multi_move", this, &Console::cmdGenmove);
...
void Console::initialize(){//*almost done  do some forward() to warmup alphazero
    ...
    if (!actor_) {
        ...
        // std::cout<<"tree_node_size"<<tree_node_size<<std::endl;
    }
    actor_->setNetwork(network_);
    
    // forward network several times to warmup since 1st few forwards requires some init time
    const int num_warmup_forward = 3;
    if (network_->getNetworkTypeName() == "alphazero") {
        std::shared_ptr<network::AlphaZeroNetwork> alphazero_network = 
            std::static_pointer_cast<network::AlphaZeroNetwork>(network_);
      for (int i = 0; i < num_warmup_forward; ++i) {
        for (int j = 0; j < config::actor_mcts_think_batch_size; ++j) { 
            alphazero_network->pushBack(actor_->getEnvironment().getFeatures()); }
...
void Console::executeCommand(std::string command){
    ...
    if (!args[0].empty() && args[0].find_first_not_of("0123456789") == std::string::npos) {
        command_id_ = args[0];  args.erase(args.begin());
    }
    // execute function
    if (function_map_.count(args[0]) == 0) { 
        return reply(ConsoleResponse::kFail, "Unknown command: " + command); }
    // cout<<"args0"<<args[0]<<endl;  ex showboard
    (*function_map_[args[0]])(args);
}
void Console::cmdGoguiAnalyzeCommands(
...
void Console::cmdPlay(const std::vector<std::string>& args){
    ...
    if (!actor_->act(act_args) && !actor_->isEnvTerminal()) {
       return reply(ConsoleResponse::kFail, "Invalid action: \"" + 
          action_string + "\""); }
    reply(ConsoleResponse::kSuccess, "");
}
...
void Console::cmdGenmove(const std::vector<std::string>& args){
    ...
    if (actor_->isEnvTerminal()) { 
        return reply(ConsoleResponse::kSuccess, "PASS"); }
    actor_->getEnvironment().setTurn(
        minizero::env::charToPlayer(args[1].c_str()[0]));
    boost::posix_time::ptime start_ptime = utils::TimeSystem::getLocalTime();
    if (args[0] == "gen_multi_move" ){
      ...
      const Action action = actor_->think(
          (args[0] == "genmove" ? true : false), true);
      std::cerr << "Spent Time = " << (utils::TimeSystem::getLocalTime() 
         - start_ptime).total_milliseconds() / 1000.0f << " (s)" << std::endl;
      if (actor_->isResign()) { 
          return reply(ConsoleResponse::kSuccess, "Resign"); }
...
void Console::calculatePolicyValue(std::vector<float>& policy, 
    float& value, utils::Rotation rotation /* = utils::Rotation::kRotationNone */)
{
    if (network_->getNetworkTypeName() == "alphazero") {
