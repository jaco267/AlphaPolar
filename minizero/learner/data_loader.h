...
public:
    ...
    inline std::shared_ptr<BatchDataPtr> getDataPtr() { 
        return std::static_pointer_cast<BatchDataPtr>(data_ptr_); }
...
protected:
    virtual bool addEnvironmentLoader();  virtual bool sampleData();
    virtual void setAlphaZeroTrainingData(int batch_index);
    virtual void setMuZeroTrainingData(int batch_index);
    inline std::shared_ptr<DataLoaderSharedData> getSharedData() { 
        return std::static_pointer_cast<DataLoaderSharedData>(shared_data_); }
};
...
public:
    ...
    std::shared_ptr<utils::BaseSlaveThread> newSlaveThread(int id) override { 
        return std::make_shared<DataLoaderThread>(id, shared_data_); }
    inline std::shared_ptr<DataLoaderSharedData> getSharedData() { 
        return std::static_pointer_cast<DataLoaderSharedData>(shared_data_); }
};
