...
PYBIND11_MODULE(minizero_py, m){
  ...
  //* added  
  m.def("get_network_type", []() { return config::network_type;}); 
  py::class_<learner::DataLoader>(m, "DataLoader")
    ...
    .def(
      "sample_data", [](learner::DataLoader& data_loader, 
        py::array_t<float>& features, py::array_t<float>& action_features, py::array_t<float>& policy, 
        py::array_t<float>& value, py::array_t<float>& reward, py::array_t<float>& loss_scale, py::array_t<int>& sampled_index) {
        data_loader.getSharedData()->getDataPtr()->features_ = 
           static_cast<float*>(features.request().ptr);
