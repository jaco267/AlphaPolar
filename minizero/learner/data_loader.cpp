...
void ReplayBuffer::addData(const EnvironmentLoader& 
  env_loader){
    std::pair<int, int> data_range = env_loader.getDataRange();
    std::deque<float> position_priorities(data_range.second + 1, 
                                          0.0f);
    float game_priority = 0.0f;
    for (int i = data_range.first; i <= data_range.second; ++i) {
        position_priorities[i] = std::pow((config::learner_use_per ? 
           env_loader.getPriority(i) : 1.0f), config::learner_per_alpha);
...
int DataLoaderSharedData::getNextBatchIndex(){
    std::lock_guard<std::mutex> lock(mutex_);
    return (batch_index_ < config::learner_batch_size ? 
         batch_index_++ : config::learner_batch_size);
}
...
bool DataLoaderThread::addEnvironmentLoader(){
    ...
    if (env_loader.loadFromString(env_string)) { 
        getSharedData()->replay_buffer_.addData(
            env_loader); }
    return true;
}
...
bool DataLoaderThread::sampleData(){
    ...
    if (batch_index >= config::learner_batch_size) { 
        return false; }
...
void DataLoaderThread::setAlphaZeroTrainingData(int batch_index){
  ...
  Rotation rotation = static_cast<Rotation>(
    Random::randInt() % static_cast<int>(Rotation::kRotateSize));
...
DataLoader::DataLoader(const std::string& conf_file_name){ //*called by pybind
    env::setUpEnv();
...
void DataLoader::initialize(){ //* called by pybind
    createSlaveThreads(config::learner_num_thread);//super() baseParaller
...
void DataLoader::loadDataFromFile(const std::string& file_name){
    ...
    for (std::string content; std::getline(fin, content);) { 
        getSharedData()->env_strings_.push_back(content); }

    for (auto& t : slave_threads_) { t->start(); }
    for (auto& t : slave_threads_) { t->finish(); }
    getSharedData()->replay_buffer_.game_priority_sum_ = std::accumulate(
        getSharedData()->replay_buffer_.game_priorities_.begin(), 
        getSharedData()->replay_buffer_.game_priorities_.end(), 
        0.0f);
}
