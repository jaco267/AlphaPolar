...
class ZeroLogger {  //* half
public:
    ZeroLogger() {}
    void createLog();
    inline void addWorkerLog(const std::string& log_str) { //*half
        addLog(log_str, worker_log_); }
    inline void addTrainingLog(const std::string& log_str) { //*half
         addLog(log_str, training_log_); }
...
class ZeroSelfPlayData { //* half
public:
    bool is_terminal_; int data_length_; int game_length_;
    float return_;
...
class ZeroWorkerSharedData {  //* half
public:
    ...
    bool is_optimization_phase_;  int num_op_worker_;
    int total_games_;  int model_iteration_;
    ZeroLogger logger_;  std::string updated_conf_str_;
    std::queue<ZeroSelfPlayData> sp_data_queue_;
    boost::mutex mutex_;  boost::mutex& worker_mutex_;
};

class ZeroWorkerHandler : public utils::ConnectionHandler {
public:
    ZeroWorkerHandler(boost::asio::io_service& io_service, 
        ZeroWorkerSharedData& shared_data)
        : ConnectionHandler(io_service),
          is_idle_(false),shared_data_(shared_data){}
...
private:
    bool is_idle_; std::string name_; std::string type_;
    ZeroWorkerSharedData& shared_data_;
};

class ZeroServer : public utils::BaseServer<ZeroWorkerHandler> {
public:
    ZeroServer(): BaseServer(minizero::config::zero_server_port),
          shared_data_(worker_mutex_),keep_alive_timer_(io_service_){
        startKeepAlive();
    }
    virtual void run();
    boost::shared_ptr<ZeroWorkerHandler> handleAcceptNewConnection() override { 
        return boost::make_shared<ZeroWorkerHandler>(io_service_, shared_data_); }
