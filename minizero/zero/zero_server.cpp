...
void ZeroLogger::createLog(){
    ...
    for (int i = 0; i < 100; ++i) {
        worker_log_ << "=";   training_log_ << "=";
    }
    worker_log_ << std::endl;   training_log_ << std::endl;
...
ZeroSelfPlayData::ZeroSelfPlayData(std::string input_data){
    //todo this is the input data from minizero/actor/actor_group.cpp/outputGame()
//   cout<<"!!!! input "<<input_data<<endl;
...
void ZeroWorkerHandler::handleReceivedMessage(
    ...
    if (args[0] == "Info") {
        ...
        if (shared_data_.sp_data_queue_.size() % 
          std::max(1, static_cast<int>(config::zero_num_games_per_iteration * 0.25)) == 0) {
            shared_data_.logger_.addTrainingLog("[SelfPlay Game Buffer] " + 
                std::to_string(shared_data_.sp_data_queue_.size()) + " games");
        }
...
void ZeroServer::run(){
    ...
    for (iteration_ = config::zero_start_iteration; 
        iteration_ <= config::zero_end_iteration; ++iteration_) {
        syncConfig();  selfPlay();   optimization();
    }
    close();
}
void ZeroServer::initialize(){ //* half just some init
...
void ZeroServer::selfPlay(){
    ...
    std::vector<int> game_lengths;  std::vector<float> game_returns;
...
void ZeroServer::broadcastSelfPlayJob(){
    ...
    for (auto& worker : connections_) {
        ...
        worker->write("load_model " + config::zero_training_directory + "/model/weight_iter_" + 
            std::to_string(shared_data_.getModelIetration()) + ".pt");
        worker->write("reset_actors");
        worker->write("start");
    }
}
void ZeroServer::optimization(){
  //* this will call  train "" -1 -1    [file],start_iter,end_iter  in minizero/learner/train.py
  shared_data_.logger_.addTrainingLog("[Optimization] Start.");
  std::string job_command = "train ";  //* this will call  train "" -1 -1    [file],start_iter,end_iter  in minizero/learner/train.py
  ...
  while (shared_data_.isOptimizationPahse()) {
      ...
      for (auto worker : connections_) {
          ...
          worker->setIdle(false);   worker->write(job_command);
      }
  }
  stopJob("op");
  shared_data_.logger_.addTrainingLog("[Optimization] Finished.");
}
std::string ZeroServer::getUpdatedConfig(){///* half  just add some config
    std::string job_command = "";
